    ...
    class UserEvents
    {
        public static async Task Discord_GuildMemberRemoved(DiscordClient client, GuildMemberRemoveEventArgs e)
        {
            new Thread(async () =>
            {
                ...
                if (member.user != null)
                {
                    var discordEmbed = Core.Discord.CreateFancyMessage(DiscordColor.IndianRed, member.clanTag + " Member left Discord server!", "User was found on spreadsheet.", new List<Field>(2) { new Field("Username", e.Member.Username + '#' + e.Member.Discriminator), new Field("ID", e.Member.Id.ToString()) });
                    Log.Information("Detected " + member.clanTag + " member leaving discord");
        ...
        public static async Task Discord_Ready(DiscordClient client, ReadyEventArgs r)
        {
            await Task.Run(() =>
            {
                ...
                AutoEvents.EventScheduler(startTimeShort, ConfigValues.configValues.LongInterval, ConfigValues.clansList, AutoEvents.AutoCheckForLeavers);
